...
public class MenuActivity extends AppCompatActivity {
    ...
    private ActivityResultLauncher<Intent> addItemLauncher = registerForActivityResult( new ActivityResultContracts.StartActivityForResult(), result -> { if (result.getResultCode() == Activity.RESULT_OK) { Intent data = result.getData(); if (data != null) { Bundle bundle = data.getExtras(); Dish newDish = null; if (bundle != null) { newDish = bundle.getParcelable("newItem", Dish.class); if (newDish != null) { menuItems.add(newDish); adapter.notifyDataSetChanged(); } } } } });
    @Override
